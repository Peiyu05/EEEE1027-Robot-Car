...
def test_movement():
    """Interactive backward movement test."""
    wait_time = 1  # seconds to run at each duty cycle
    print("Interactive Backward Movement Test")
    ...
    try:
        while True:
            ...
            print(f"→ Running backward at {duty}% for {wait_time}s...")
            backward(duty)
            time.sleep(wait_time)
            stop()
            input("Measure distance traveled, then press Enter to continue...\n")

        print("Backward Movement Test Complete!")
